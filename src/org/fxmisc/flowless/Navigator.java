...
extends Region implements TargetPositionVisitor {
    ...
    private void cropToNeighborhoodOf(int itemIndex, double additionalOffset) {
        ...
        int itemsBefore;
        int itemsAfter ;
        if (!avgLen.isPresent() || Math.abs(avgLen.get()) < 1e-6) {
            itemsBefore = 5;
            itemsAfter = 5;
        } else {
            itemsBefore = avgLen.map(l -> spaceBefore / l).orElse(5.0).intValue();
            itemsAfter = avgLen.map(l -> spaceAfter / l).orElse(5.0).intValue();
        }
        int crop0 = itemIndex - itemsBefore;
        int crop1= itemIndex + 1 + itemsAfter;
        positioner.cropTo(crop0, crop1 );
    }
